async function analyze() {
  const guestScanUsed = localStorage.getItem("guestScanUsed");

  if (guestScanUsed) {
    alert("Free scan already used. Please login to continue.");
    return;
  }
  ...
  renderResult(data);
  localStorage.setItem("guestScanUsed", "true");
}
...
function demo() {
  const example = {
    data: {
      ...
      decisionExplanation: {
        primaryReason:
          "High severity vulnerabilities detected in production dependencies.",
